...
class GraphRetriever:
    ...
    def __init__(self, db: Session, project_id: str, as_of=None):
        """
        Args:
            as_of: Optional datetime for point-in-time graph queries.
                   None (default) = only active edges (valid_until IS NULL).
        """
        ...
        self._load_graph(as_of=as_of)

    def _load_graph(self, as_of: object = None):
        """Load nodes and edges for this project into memory.

        Bi-temporal filtering:
          - CO_OCCURS edges: always loaded (historical co-occurrence is a permanent fact)
          - Typed edges (WORKS_AT, MEMBER_OF, etc.): only load currently-active edges
            (valid_until IS NULL), unless as_of is supplied for point-in-time queries.
        """
        from app.services.knowledge_graph.graph_builder import SUPERSEDABLE_RELATIONS
        ...
        # Load edges with bi-temporal filtering
        all_edges = self._db.query(self._models.EntityEdge).filter(
            self._models.EntityEdge.project_id == self._project_id
        ).all()

        for edge in all_edges:
            # For supersedable typed edges, skip if superseded (valid_until is set)
            if edge.relation_type in SUPERSEDABLE_RELATIONS:
                if as_of is not None:
                    # Point-in-time: skip edges not yet valid or already expired
                    if edge.valid_from and edge.valid_from > as_of:
                        continue
                    if edge.valid_until and edge.valid_until <= as_of:
                        continue
                else:
                    # Default: only active edges (valid_until IS NULL)
